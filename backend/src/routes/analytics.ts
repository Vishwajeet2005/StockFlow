...
router.get('/', (req: AuthRequest, res: Response) => {
  ...
  const orders = queryAll("SELECT * FROM orders WHERE company_id=? AND status='completed'", [c]);
  ...
  orders.forEach((o: any) => {
    const orderDate = new Date(o.date + (o.date.includes('T') ? '' : 'Z'));
    ...
    if (o.type === 'sale') {
      ...
      if (monthlySales[monthStr] !== undefined) {
        monthlySales[monthStr] += o.total_amount;
      }
