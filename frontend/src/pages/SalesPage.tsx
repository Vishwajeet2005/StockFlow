...
export default function SalesPage() {
  ...
  const handleAdvance = async (order: Order) => {
    ...
    if (idx >= statuses.length - 1) return toast('Order is already completed');
    try {
    ...
    } catch (e: any) { toast.error(e.response?.data?.error || 'Failed to advance order'); }
  };
  ...
  const nextStatus = (order: Order) => {
    const idx = saleStatuses.indexOf(order.status);
    return idx < saleStatuses.length - 1 ? statusLabel[saleStatuses[idx + 1]] : null;
  };
  ...
  return (
    <div className="h-full flex flex-col">
        ...
        <div className="list-panel flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? <div className="p-4 text-center text-sm text-ink-300">Loading…</div>
              : orders.length === 0 ? <div className="empty-state"><ShoppingCart size={28} /><p>No sales orders</p></div>
              : orders.map(o => (
                <div key={o.order_id} className={`list-item ${selected?.order_id === o.order_id ? 'selected' : ''}`} onClick={() => setSelected(o)}>
                  ...
                  <div className="text-sm text-ink-500 truncate">{o.party_name || 'No customer'}</div>
            ...
            <div className="p-6 animate-fade-in">
              <div className="flex items-start justify-between mb-5">
                <div>
                  ...
                  <p className="text-sm text-ink-400 mt-0.5">{selected.party_name || 'No customer'} · {fmt.datetime(selected.date)}</p>
                </div>
              ...
              <div className="card p-4 mb-4">
                <div className="flex items-center gap-1">
                  {saleStatuses.map((s, i) => {
                    const current = saleStatuses.indexOf(selected.status);
                    ...
                    return (
                      <React.Fragment key={s}>
                        ...
                        {i < saleStatuses.length - 1 && <div className={`flex-1 h-0.5 ${i < current ? 'bg-brand-500' : 'bg-surface-3'}`} />}
                      </React.Fragment>
                    );
                  })}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div className="card p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between"><span className="text-ink-500">Customer</span><span className="text-ink-900 font-medium">{selected.party_name || '—'}</span></div>
                    <div className="flex justify-between"><span className="text-ink-500">Customer ID</span><span className="text-ink-900 font-mono text-xs">{selected.party_id || '—'}</span></div>
      ...
      {showForm && (
        <OrderForm
          type="sale"
          onSave={order => { setOrders(prev => [order, ...prev]); setSelected(order); setShowForm(false); }}
      ...
      {confirmDelete && (
        <ConfirmDialog
          title="Delete Order"
          message={`Delete order ${selected?.order_id}? This cannot be undone.`}
          confirmLabel="Delete"
          danger
