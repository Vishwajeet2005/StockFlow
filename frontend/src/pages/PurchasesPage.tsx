...
import api, { Order, fmt, statusLabel, statusBadge, purchaseStatuses, PaginatedResponse } from '../lib/api';
...
export default function PurchasesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/orders', { params: { type: 'purchase', search: search || undefined, page, limit: 20 } });
      setOrders(data.data);
      setMeta(data.meta);
